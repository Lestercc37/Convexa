...
def test_vanna_exposure_migration_adds_mandatory_column(monkeypatch) -> None:
    migration = import_module("backend.db.migrations.0011_add_vanna_exposure")
    added_columns = []
    altered_columns = []
    monkeypatch.setattr(
        migration.op,
    ...
    table, column = added_columns[0]
    assert table == "gamma_aggregates"
    assert column.name == "vanna_exposure"
    assert column.nullable is False
    assert altered_columns == [("gamma_aggregates", "vanna_exposure", {"server_default": None})]


def test_delta_exposure_migration_adds_mandatory_column(monkeypatch) -> None:
    migration = import_module("backend.db.migrations.0027_add_delta_exposure")
    added_columns = []
    altered_columns = []
    monkeypatch.setattr(
        migration.op,
    ...
    table, column = added_columns[0]
    assert table == "gamma_aggregates"
    assert column.name == "delta_exposure"
    assert column.nullable is False
    assert altered_columns == [("gamma_aggregates", "delta_exposure", {"server_default": None})]
