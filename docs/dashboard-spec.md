...
- **2026-09-16: Vega/Theta Exposure conectados al panel, y se agregó Delta Exposure (DEX).** La asimetría descrita arriba (Vega/Theta calculados pero nunca mostrados) se cerró agregando sus tarjetas junto a Charm/Vanna, mismo `exposure-panel`. Delta Exposure es una métrica nueva, mismo patrón exacto de agregación que las otras cuatro (`Σ(Delta × OI × 100)`, sin desglose por strike todavía) — a diferencia de gamma/vanna/charm (BSM), delta es un valor real cotizado por ThetaData, así que no requiere ni admite un ajuste de signo call/put adicional: su propio signo ya distingue calls (positivo) de puts (negativo). Hereda automáticamente el patrón de todos-los-vencimientos para índices (PR #132) por construcción, al calcularse en el mismo `CalculateGammaExposureOrchestrator.execute()` que las demás.
