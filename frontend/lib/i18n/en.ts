...
export const en: Translations = {
  ...
  dashboard: {
    ...
    gammaViewGroupAriaLabel: "Gamma View",
  ...
  chartSecondaryPanel: {
    ...
    gexLevelsLegendAriaLabel: "GEX structural levels",
  ...
  enginesGuide: {
    ...
    engines: {
      ...
      gammaFlip: {
        name: "Gamma Flip",
        description:
          "The market's regime boundary: price above it means Long Gamma (dampens volatility), below it means Short Gamma (amplifies it). If price already crossed it since the last recalculation, the regime is marked \"price-based\", lower confidence. Convexa picks the sign-crossing strike closest to the current spot price, not simply the first one found scanning from the lowest strike — confirmed live (AAPL, 2026-09-18) that a single low-magnitude, noisy strike far from spot could otherwise relocate the level by double-digit points with no real market move. A minimum-magnitude filter was tested and rejected: it helped that case but made a large index's level worse and nearly eliminated another's coverage, so it doesn't generalize across underlyings. Two other suspected issues were independently re-verified against the calculation code and are correct as implemented: the dealer sign (+1 call / -1 put) is applied to the exposure, not to gamma itself (mathematically identical for calls and puts), and every contract uses its own individual implied volatility, never a chain-wide average. Known limitation, not yet addressed: each contract's gamma is priced once, at the real current spot, rather than swept across a range of hypothetical spot prices the way SpotGamma's own Zero Gamma level is — a smoother but substantially larger calculation change.",
      },
      absoluteGammaStrike: {
