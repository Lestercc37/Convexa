...
export const es: Translations = {
  ...
  priceChart: {
    ...
    expectedMoveRangeLabel: "Movimiento Esperado",
  ...
  enginesGuide: {
    ...
    engines: {
      ...
      gammaFlip: {
        name: "Gamma Flip",
        description:
          "El límite de régimen del mercado: precio por encima indica Long Gamma (amortigua volatilidad), por debajo indica Short Gamma (la amplifica). Si el precio ya lo cruzó antes del último recálculo, el régimen se marca como \"basado en precio\", de menor confianza. Convexa elige el strike de cruce de signo más cercano al precio spot actual, no simplemente el primero encontrado escaneando desde el strike más bajo — confirmado en vivo (AAPL, 2026-09-18) que un único strike de magnitud pequeña y ruidosa, lejos del spot, podía reubicar el nivel varios puntos sin ningún cambio real de mercado. Se evaluó y descartó un filtro de magnitud mínima: ayudaba en ese caso pero empeoraba el nivel de un índice grande y casi eliminaba la cobertura de otro, por lo que no generaliza entre distintos subyacentes. Otras dos sospechas se re-verificaron de forma independiente contra el código de cálculo y están correctas tal como están implementadas: el signo de dealer (+1 call / -1 put) se aplica sobre la exposición, no sobre el gamma en sí (matemáticamente idéntico para calls y puts), y cada contrato usa su propia volatilidad implícita individual, nunca un promedio de toda la cadena. Limitación conocida, aún no resuelta: el gamma de cada contrato se calcula una sola vez, al spot real actual, en vez de barrerse sobre un rango de precios spot hipotéticos como hace el propio Zero Gamma de SpotGamma — un cálculo más suave, pero un cambio sustancialmente mayor.",
      },
      absoluteGammaStrike: {
